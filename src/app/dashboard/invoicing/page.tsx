    ...
    <div className="space-y-6">
        ...
        <Card>
          <CardContent className="p-4">
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-sm text-surface-9000">Draft</span>
            </div>
            <p className="text-xl font-bold">{summary?.totalDraft ?? 0}</p>
            <p className="text-xs text-surface-500">{formatCurrency(summary?.amountDraft ?? 0)}</p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-4">
            <div className="flex items-center gap-2 mb-1">
              <Send className="h-4 w-4 text-status-info-400" />
              <span className="text-sm text-surface-9000">Sent</span>
            </div>
            <p className="text-xl font-bold text-status-info-600">{summary?.totalSent ?? 0}</p>
            <p className="text-xs text-surface-500">{formatCurrency(summary?.amountSent ?? 0)}</p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-4">
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-sm text-surface-9000">Paid</span>
            </div>
            <p className="text-xl font-bold text-status-success-600">{summary?.totalPaid ?? 0}</p>
            <p className="text-xs text-surface-500">{formatCurrency(summary?.amountPaid ?? 0)}</p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-4">
            <div className="flex items-center gap-2 mb-1">
              <AlertCircle className="h-4 w-4 text-status-error-400" />
              <span className="text-sm text-surface-9000">Overdue</span>
            </div>
            <p className="text-xl font-bold text-status-error-600">{summary?.totalOverdue ?? 0}</p>
            <p className="text-xs text-surface-500">{formatCurrency(summary?.amountOverdue ?? 0)}</p>
          </CardContent>
        </Card>
      </div>
          ...
          {loading ? (
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {invoices.map(inv => (
                    <tr key={inv.id} className="border-b last:border-0 hover:bg-surface-900 cursor-pointer" onClick={() => setSelectedInvoice(inv)}>
                      ...
                      <td className="py-3">
                        {inv.customer ? (
                          <div>
                            ...
                            <span className="text-xs text-surface-500 ml-2">{inv.customer.pmbNumber}</span>
                          </div>
                        ) : '—'}
                      </td>
                      <td className="py-3 text-right font-mono font-medium">
                        ...
                        {inv.amountPaid > 0 && inv.status !== 'paid' && (
                          <div className="text-xs text-status-success-500">Paid: {formatCurrency(inv.amountPaid)}</div>
                        )}
                      </td>
                      <td className="py-3">
                      ...
                      <td className="py-3" onClick={e => e.stopPropagation()}>
                        <div className="flex gap-1">
                          ...
                          {inv.status !== 'paid' && inv.status !== 'void' && isAdmin && (
                            <Button variant="ghost" size="sm" onClick={() => handleVoidInvoice(inv.id)} className="text-status-error-500">
          ...
          <div className="p-4 space-y-4">
            <div className="flex justify-between items-start">
              <div>
                ...
                {selectedInvoice.customer?.pmbNumber && (
                  <p className="text-sm text-surface-9000">{selectedInvoice.customer.pmbNumber}</p>
                )}
              </div>
            ...
            {selectedInvoice.lineItems.length > 0 && (
              <div>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b text-surface-9000">
                  ...
                  <tfoot>
                    ...
                    {selectedInvoice.tax > 0 && (
                      <tr>
                        <td colSpan={3} className="text-right text-surface-9000">Tax</td>
                        <td className="text-right font-mono text-surface-9000">{formatCurrency(selectedInvoice.tax)}</td>
                      </tr>
                    )}
            ...
            <div className="grid grid-cols-2 gap-4 text-sm">
              <div>
                <span className="text-surface-9000">Due Date:</span>{' '}
                {selectedInvoice.dueDate ? formatDate(selectedInvoice.dueDate) : '—'}
              </div>
              <div>
                <span className="text-surface-9000">Created:</span>{' '}
                {formatDateTime(selectedInvoice.createdAt)}
              </div>
              {selectedInvoice.paidAt && (
                <div>
                  <span className="text-surface-9000">Paid:</span>{' '}
                  {formatDateTime(selectedInvoice.paidAt)}
                </div>
              )}
              {selectedInvoice.sentAt && (
                <div>
                  <span className="text-surface-9000">Sent:</span>{' '}
            ...
            {selectedInvoice.notes && (
              <p className="text-sm text-surface-9000 bg-surface-900 rounded p-2">{selectedInvoice.notes}</p>
            )}
          </div>
        )}
      </Modal>
      ...
      >
        <div className="p-4 space-y-4">
          ...
          <p className="text-sm text-surface-9000">
            {genMode === 'single'
