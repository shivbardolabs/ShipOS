      ...
      try {
        ...
        if (res.ok) {
          ...
          if (!cancelled && Array.isArray(data.customers)) {
            // Merge DB customers with mock data — DB customers take priority
            const dbIds = new Set(data.customers.map((c: Customer) => c.id));
            const merged = [
              ...data.customers,
              ...mockCustomers.filter((m) => !dbIds.has(m.id)),
            ];
            setCustomers(merged);
          }
        }
