...
async function aggregateStoreData(user: { id: string; role: string; tenantId: string | null }) {
  const now = new Date();
  ...
  return {
    ...
    agingPackageDetails: agingPackages.map((p) => ({
...
async function getDemoBriefing(user: { id: string; role: string; tenantId: string | null }): Promise<Briefing> {
  ...
  return {
    ...
    actionItems: [
      ...(data.agingPackages > 0
        ? [{ text: `Send pickup reminders for ${data.agingPackages} aging packages (7+ days)`, href: '/dashboard/packages', priority: 'high' as const }]
        : []),
      ...(data.expiringIds > 0
        ? [{ text: `${data.expiringIds} customer IDs expiring within 30 days — review compliance`, href: '/dashboard/compliance', priority: 'high' as const }]
        : []),
      ...(data.pendingShipments > 0
        ? [{ text: `Process ${data.pendingShipments} pending shipments before end of day`, href: '/dashboard/shipping', priority: 'medium' as const }]
        : []),
      ...(data.inactiveCustomers > 0
        ? [{ text: `Re-engage ${data.inactiveCustomers} inactive customers (no activity in 14+ days)`, href: '/dashboard/customers', priority: 'medium' as const }]
        : []),
      { text: 'Review daily revenue breakdown and outstanding invoices', href: '/dashboard/invoicing', priority: 'low' as const },
    ],
    metrics: [
      { label: 'Packages Held', value: String(data.totalPackagesHeld), trend: 'flat' as const },
      { label: 'Checked In Today', value: String(data.checkedInToday), trend: 'up' as const },
      { label: 'Revenue Today', value: `$${data.revenueToday.toFixed(2)}`, trend: 'up' as const },
      { label: 'Notifications Sent', value: String(data.notificationsSent), trend: 'flat' as const },
    ],
  ...
  const response = await fetch('https://api.openai.com/v1/chat/completions', {
    ...
    body: JSON.stringify({
      ...
      messages: [
        ...
        { role: 'user', content: `Here is today's store data:\n\n${JSON.stringify(data, null, 2)}` },
      ],
      max_tokens: 1000,
      temperature: 0.7,
    }),
