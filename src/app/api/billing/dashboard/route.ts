...
export const GET = withApiHandler(async (request: NextRequest, { user }) => {
  ...
  if (query.from && query.to) {
    ...
    const range = dateRangeFromGranularity(query.period);
    dateFrom = range.from;
    dateTo = range.to;
  }
  ...
  // Base where clause
  const baseWhere: Prisma.ChargeEventWhereInput = { tenantId };
  if (query.customerId) baseWhere.customerId = query.customerId;
  if (query.serviceType) baseWhere.serviceType = query.serviceType;

  const currentWhere: Prisma.ChargeEventWhereInput = {
    ...baseWhere,
    createdAt: { gte: dateFrom, lte: dateTo },
  };

  const prevWhere: Prisma.ChargeEventWhereInput = {
    ...baseWhere,
