import { withApiHandler, validateBody, ok, forbidden } from '@/lib/api-utils';
...
export const DELETE = withApiHandler(async (_request, { user }) => {
  if (!['admin', 'superadmin'].includes(user.role)) {
    return forbidden('Admin required');
  }
  ...
  return ok({ success: true, message: 'Demo data removed' });
});
